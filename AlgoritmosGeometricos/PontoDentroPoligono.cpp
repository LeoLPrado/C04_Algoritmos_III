...
int main(){
	...
	Ponto ponto_pesquisado;
	cin >> ponto_pesquisado.x >> ponto_pesquisado.y;
	
	int ref = orientacao(pontos[0], pontos[1], ponto_pesquisado);
    bool dentro = true;

    for (int i = 1; i < n; i++) {
        int o = orientacao(pontos[i], pontos[(i+1) % n], ponto_pesquisado);
        if (o != ref) {
            dentro = false;
            break;
        }
